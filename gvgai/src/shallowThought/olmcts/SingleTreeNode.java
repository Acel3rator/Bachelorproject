...
public class SingleTreeNode
{
    ...
    public SingleTreeNode(SingleTreeNode parent, int childIdx, Random rnd) {
        this.parent = parent;
        this.m_rnd = rnd;
        children = new SingleTreeNode[OLMCTSAgent.NUM_ACTIONS];
        totValue = 0.0;
    ...
    public void mctsSearch(ElapsedCpuTimer elapsedTimer) {
        ...
        //while(numIters < OLMCTSAgent.MCTS_ITERATIONS){
    ...
    public SingleTreeNode treePolicy(StateObservation state) {
        ...
        while (!state.isGameOver() && cur.m_depth < OLMCTSAgent.ROLLOUT_DEPTH)
        {
    ...
    public SingleTreeNode expand(StateObservation state) {
        ...
        //Roll the state
        state.advance(OLMCTSAgent.actions[bestAction]);
    ...
    public SingleTreeNode uct(StateObservation state) {
        ...
        for (SingleTreeNode child : this.children)
        {
            ...
            double uctValue = childValue +
                    OLMCTSAgent.K * Math.sqrt(Math.log(this.nVisits + 1) / (child.nVisits + this.epsilon));
        ...
        //Roll the state:
        state.advance(OLMCTSAgent.actions[selected.childIdx]);
    ...
    public double rollOut(StateObservation state)
    {
        ...
        while (!finishRollout(state,thisDepth)) {
            ...
            int action = m_rnd.nextInt(OLMCTSAgent.NUM_ACTIONS);
            state.advance(OLMCTSAgent.actions[action]);
            thisDepth++;
        }
    ...
    {
        if(depth >= OLMCTSAgent.ROLLOUT_DEPTH)      //rollout end condition.
            return true;
